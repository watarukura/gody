...
func extractValidationErrors(err error) error {
	if err != nil {
		...
		for _, err := range err.(validator.ValidationErrors) {
			errorText = append(errorText, validationErrorText(err))
		}
...
func validationErrorText(e validator.FieldError) string {
	f := e.Field()
	switch e.Tag() {
	case "required":
	...
	case "min":
		return fmt.Sprintf("%s cannot be less than %s", f, e.Param())
	}
